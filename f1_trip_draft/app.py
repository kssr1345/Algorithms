...
def recommend_trips(user: UserPreferences, trips: List[TripCandidate]) -> List[Dict]:
    ...
    for trip in trips:
        ...
        scores = {
            "value": value_score(trip.total_cost, trip.experience_points),
            "experience": min(100.0, float(trip.experience_points)),
            "weather": weather_fit(
        ...
        recommendations.append(
            {
                ...
                "save_tips": [
                    "Fly mid-week and arrive Thursday to reduce peak fare spikes.",
                    "Stay 3-7 km from city center and use train/metro for better value.",
                ],
                "splurge_tips": [
                    "Spend extra on one premium day activity for the highest experience jump.",
                    "Choose one premium add-on: museum pass, guided food tour, or landmark experience.",
                ],
            }
        )
...
def sample_data() -> List[TripCandidate]:
    return [
        TripCandidate(
            ...
            flight_hours=2.1,
            transfer_minutes=55,
            experience_points=88,
        ),
        TripCandidate(
            name="Nagoya Culture Week",
            country="Japan",
            city="Nagoya",
...
def build_user_preferences_interactive() -> UserPreferences:
    print("\nWelcome to the PlanWise travel draft!\n")
...
def print_recommendations(user: UserPreferences, recos: List[Dict]) -> None:
    ...
    for idx, item in enumerate(recos[:3], start=1):
        ...
        print(
            ...
            f"Experience {item['scores']['experience']:.1f}, Weather {item['scores']['weather']:.1f}, "
